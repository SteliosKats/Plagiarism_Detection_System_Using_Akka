    ...
    if (!references_ar.isEmpty){
     // println("references_ar.length:"+references_ar.length+"\t references_de.length:"+references_ar.length)
      if(references_ar.length==1 && references_de.isEmpty ) {
        val new_reference_array=(line.substring(references_ar(references_ar.length -1),line.length()))
        ...
        println(references_ar(references_ar.length -1)+"\t line length:"+(line.length()-1))
        println(new_reference_array)
      }
