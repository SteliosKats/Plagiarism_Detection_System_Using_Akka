...
class FragFeaturesCalc extends Actor with ActorLogging {
  ...
  def receive ={
    case calculate_wks(plag_file_matches,source_file_matches,normalised_term_frequency,normalised_source_term_freq,term_files_occ,id_total,compared_tuple_w_ids) =>
      var start :Int = -1
      val compared_ids :Tuple2[Int,Int]=(compared_tuple_w_ids._3,compared_tuple_w_ids._4)
      for(key1 <- source_file_matches.iterator){     ///gia kathe koinh leksh tou source file
        for(position <- key1._2.iterator ) {        //gia kathe thesh pou vrethike h koinh leksh sto source file
          for (plag_iter <- plag_file_matches.iterator if (plag_iter._1 == key1._1)) {
            find_first_seq(position, plag_iter, plag_file_matches, source_file_matches,compared_ids) //briskoume thn prwth emfanish ths sto plag_file
          } //kai ekteloume th synarthsh pou tha mas dwsei thn prwth koinh akolouthia
        }
      }
      router.route(Broadcast(Terminate_FR_Calcs(source_file_matches,plag_file_matches,normalised_term_frequency,normalised_source_term_freq,term_files_occ,id_total,compared_tuple_w_ids)), sender())

    case _ => println("Wrong Data Sent To FragFeaturesCalc Actor!")
  }

  def find_first_seq(source_key_pos:Int,plag_iter :(String,mutable.Set[Int]),plag_file_matches :HashMap[String, scala.collection.mutable.Set[Int]] with scala.collection.mutable.MultiMap [String,Int],source_file_matches :HashMap[String, scala.collection.mutable.Set[Int]] with scala.collection.mutable.MultiMap [String,Int],compared_ids :Tuple2[Int,Int]):Unit ={
    ...
    for (pos_found <- plag_iter._2.iterator){     //h thesi sthn opoia vrethike h koinh leksh metaksy source kai plagiarised document (sto plagiarised document)
      ...
      router.route(calculate_wks2(start,end,seq_start,plag_file_matches,source_key_pos,source_file_matches,compared_ids),sender())   //stelnoume tis lekseis se routees gia thn pio grhgorh dhmiourgia twn koinwn string metaksy twn dyo keimenwn
    }
  }
...
class FragmentationCalcInception extends Actor with ActorLogging {
  def receive ={
    case calculate_wks2(start,end,seq_start,plag_file_matches,source_key_pos,source_file_matches,compared_ids) =>
      //println(seq_start)
      var start_end_fltr :Array[Int]= new Array[Int](8)
      ...
      start_end_fltr(2)=1    //synthiki termatismou ths while (einai 1 otan ektelestike ena apo ta 2 if sth for ths synrathshs pou kaleitai)
      ...
      start_end_fltr(6)=compared_ids._1     //source_id
      start_end_fltr(7)=compared_ids._2     //suspicious file (plag_file) id
      var seq_str :String=seq_start         //h prwth leksh pou vrethike koinh sta 2 sygkrinomena keimena
      ...
      context.actorSelection("/user/plag_analysis/comparing_s_p/returned_matches/classification/fragmentation_features").!(frag_calculate(seq_str,start_end_fltr))

    case Terminate_FR_Calcs(source_file_matches,plag_file_matches,normalised_term_frequency,normalised_source_term_freq,term_files_occ,id_total,compared_tuple_w_ids) =>
      context.actorSelection("/user/plag_analysis/comparing_s_p/returned_matches/classification/fragmentation_features").!(FR_Calcs_Routees_Terminated(source_file_matches,plag_file_matches,normalised_term_frequency,normalised_source_term_freq,term_files_occ,id_total,compared_tuple_w_ids))
  ...
  def find_whole_seq(start_end_fltr :Array[Int] ,seq_str :String ,plag_file_matches :HashMap[String, scala.collection.mutable.Set[Int]] with scala.collection.mutable.MultiMap [String,Int],source_file_matches :HashMap[String, scala.collection.mutable.Set[Int]] with scala.collection.mutable.MultiMap [String,Int]):(String,Array[Int]) = {
    ...
    //if(start_end_fltr(0) >=299 && start_end_fltr(1)<=450)
      //println(seq_str2+" and " +start_end_fltr(3))
